...
const AllFolders = () => {
    ...
    const fetchFolderContents = async () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          setFiles(
            (data.files || []).map((file: FileItem) => ({ // Explicitly type the file parameter
              ...file,
              folder: currentPath,
        ...
        setError(err instanceof Error ? err.message : "An unknown error occurred"); // Proper error handling
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleFileDelete = (deletedFileName: string, folder: string) => {
    setFiles(prevFiles => prevFiles.filter(file => 
      !(file.name === deletedFileName && file.folder === folder)
    ));
  };
