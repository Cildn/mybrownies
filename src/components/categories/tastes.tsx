...
interface ProductType {
  ...
  flavors: string[]; // Changed to flavors
  sizes: string[];
  prices: number[];
  isFeatured: boolean;
}
  ...
  const filteredProducts = useMemo(() => {
    return allProducts
      .filter((p: ProductType) => {
        ...
        if (filters.flavors.length && !p.flavors.some((f) => filters.flavors.includes(f))) return false;
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <Sidebar
          ...
          onClearAll={() =>
            setFilters({ flavors: [], sizes: [], priceRange: null, sortBy: undefined })
          }
        />
