...
interface TextareaProps {
  ...
  rows?: number;
  value?: string;
  onChange?: (value: string) => void; // Expects a value, not an event
  className?: string;
  disabled?: boolean;
  error?: boolean;
  hint?: string;
...
const TextArea: React.FC<TextareaProps> = ({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    if (onChange) {
      onChange(e.target.value); // Passes the value directly
    }
  };
      ...
      {hint && (
        <p
          className={`mt-2 text-sm ${
            error ? "text-error-500" : "text-black dark:text-white"
          }`}
        >
          {hint}
        </p>
